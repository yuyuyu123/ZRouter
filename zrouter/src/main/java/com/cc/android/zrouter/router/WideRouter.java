...
public class WideRouter {
    ...
    private WideRouter(ZRouterApplication context) {
    ...
    public static synchronized WideRouter getInstance(@NonNull ZRouterApplication context) {
        if (sInstance == null) {
    ...
    public RouterResponse route(String domain, String routerRequest) {
        ...
        if (PROCESS_NAME.equals(domain)) {
            ZRouterActionResult result = new ZRouterActionResult.Builder()
                    .code(ZRouterActionResult.CODE_TARGET_IS_WIDE)
        ...
        if (null == target) {
            if (!connectLocalRouter(domain)) {
                ZRouterActionResult result = new ZRouterActionResult.Builder()
                        .code(ZRouterActionResult.CODE_ROUTER_NOT_REGISTER)
                ...
                while (true) {
                    ...
                    if (time >= 600) {
                        ZRouterActionResult result = new ZRouterActionResult.Builder()
                                .code(ZRouterActionResult.CODE_CANNOT_BIND_LOCAL)
        ...
        try {
            ...
            e.printStackTrace();
            ZRouterActionResult result = new ZRouterActionResult.Builder()
                    .code(ZRouterActionResult.CODE_REMOTE_EXCEPTION)
                    .msg(e.getMessage())
                    .build();
            routerResponse.mResultString = result.toString();
            return routerResponse;
